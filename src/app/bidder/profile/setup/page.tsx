...
export default function OrgSetupPage() {
  ...
  const input = "w-full px-3 py-2.5 border border-[var(--border)] rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent";
  const label = "block text-sm font-medium text-[var(--foreground)] mb-1";

  if (loading) {
    return (
      <div className="min-h-screen bg-[var(--background)] flex items-center justify-center">
        <div className="text-sm text-[var(--foreground-tertiary)] animate-pulse">Loading...</div>
      </div>
    );
  }
  ...
  if (hasOrg) {
    return (
      <div className="min-h-screen bg-[var(--background)] flex items-center justify-center px-4">
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-sm p-8 w-full max-w-md text-center">
          <CheckCircle2 className="w-12 h-12 text-[var(--green-600)] mx-auto mb-4" />
          <h2 className="text-xl font-bold text-[var(--foreground)] mb-2">Organization Already Set Up</h2>
          <p className="text-sm text-[var(--foreground-tertiary)] mb-6">Your organization profile is already complete. You can start bidding on tenders.</p>
          <a href={redirect} className="inline-flex items-center gap-2 bg-[var(--navy-800)] text-white font-medium px-6 py-3 rounded-lg hover:bg-[var(--navy-700)] text-sm">
  ...
  if (success) {
    return (
      <div className="min-h-screen bg-[var(--background)] flex items-center justify-center px-4">
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-sm p-8 w-full max-w-md text-center">
          <div className="w-16 h-16 bg-[var(--green-100)] rounded-full flex items-center justify-center mx-auto mb-4">
            <CheckCircle2 className="w-8 h-8 text-[var(--green-600)]" />
          </div>
          <h2 className="text-xl font-bold text-[var(--foreground)] mb-2">Profile Created!</h2>
          <p className="text-sm text-[var(--foreground-tertiary)] mb-6">Your organization profile has been saved. Redirecting you to continue your bid...</p>
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] py-8 px-4">
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <Link href={redirect} className="text-sm text-[var(--foreground-tertiary)] hover:text-[var(--foreground-secondary)] flex items-center gap-1 mb-4">
            <ArrowLeft className="w-3.5 h-3.5" /> Back
          </Link>
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-[var(--navy-800)] rounded-xl flex items-center justify-center">
              <Building2 className="w-7 h-7 text-[var(--foreground)]" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-[var(--foreground)]">Complete Your Organization Profile</h1>
              <p className="text-sm text-[var(--foreground-tertiary)] mt-1">Required to submit bids on government tenders</p>
            </div>
          </div>
        </div>

        {/* Info banner */}
        <div className="bg-[var(--navy-100)] border border-blue-200 rounded-xl p-4 mb-6">
          <div className="flex items-start gap-3">
            <Shield className="w-5 h-5 text-[var(--navy-600)] mt-0.5 shrink-0" />
            <div>
              <p className="text-sm font-medium text-blue-900">Why do we need this?</p>
              <p className="text-xs text-[var(--navy-600)] mt-1">
        ...
        {error && <div className="mb-4 bg-[var(--red-100)] border border-[#F5CFCA] text-[var(--danger)] text-sm rounded-lg px-4 py-3">{error}</div>}

        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Organization Details */}
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
            <h2 className="font-semibold text-[var(--foreground)] mb-4 flex items-center gap-2">
              <Building2 className="w-4 h-4 text-[var(--navy-600)]" /> Organization Details
            </h2>
          ...
          {/* Tax & Registration */}
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
            <h2 className="font-semibold text-[var(--foreground)] mb-4">Tax & Registration Numbers</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
          ...
          <button type="submit" disabled={saving} className="w-full flex items-center justify-center gap-2 bg-[var(--navy-800)] text-white font-medium py-3 rounded-lg hover:bg-[var(--navy-700)] text-sm disabled:opacity-50">
